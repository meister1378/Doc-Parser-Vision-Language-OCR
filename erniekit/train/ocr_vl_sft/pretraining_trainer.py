...
class PretrainingTrainer(Trainer):
    ...
    def get_eval_dataloader(self, eval_dataset=None):
        """
        Eval dataloader aligned with train:
        - DistDataLoader when TP>1 or PP>1
        - MMDataloader otherwise
        """
        ...
        if (
            ...
            return DistDataLoader(
                ...
                is_train_text=getattr(self, "is_train_text", False),
                text_sft_dataset=getattr(self, "text_sft_dataset", None),
                need_data=self.args.need_data,
                pp_broadcast=not self.args.pp_need_data_degree,
                gradient_accumulation_steps=self.args.gradient_accumulation_steps,
                modality_ratio=getattr(self, "modality_ratio", [1, 1]),
                is_train_mm=getattr(self, "is_train_mm", True),
    ...
    def _maybe_log_save_evaluate(
        ...
        metrics = None
        if self.control.should_evaluate and getattr(self, "eval_dataset", None) is not None:
