    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                # Debug: show original decoded texts for the first batch to understand labels vs template
                if (
                    ...
                    try:
                        ...
                        input_ids_np = (
                            inputs["input_ids"]
                            if isinstance(inputs["input_ids"], _np.ndarray)
                            else _np.asarray(inputs["input_ids"])
                        )
                        labels_np = (
                            inputs["labels"]
                            if isinstance(inputs["labels"], _np.ndarray)
                            else _np.asarray(inputs["labels"])
                        )
                        ignored_index = getattr(self.tokenizer, "ignored_index", -100)
                        max_show = min(2, input_ids_np.shape[0])
                        for i in range(max_show):
                            ids_i = input_ids_np[i].tolist()
                            lbl_i = labels_np[i].tolist()
                            full_text = self.tokenizer.decode(
                                ids_i, skip_special_tokens=False
                            )
                            valid_ids = [int(t) for t in lbl_i if t != ignored_index]
                            label_text = (
                            ...
                            valid_idx = [j for j, t in enumerate(lbl_i) if t != ignored_index]
                            span_info = (
                                (valid_idx[0], valid_idx[-1])
                                if len(valid_idx) > 0
                                else None
                            )
                            print(f"[DEBUG:ORIGINAL] sample={i}")
                            print(f"[DEBUG:ORIGINAL] full_input_text: {full_text[:100000]}")
                            print(f"[DEBUG:ORIGINAL] label_text       : {label_text[:100000]}")
                            print(f"[DEBUG:ORIGINAL] num_valid_labels : {len(valid_idx)}, span={span_info}")
                    except Exception as _e:
                        print(f"[DEBUG:ORIGINAL] decode failed: {_e}")
