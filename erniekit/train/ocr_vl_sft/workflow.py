...
def run_ocr_vl_sft(
    ...
    image_preprocess_save = SiglipImageProcessor.from_pretrained(
    ...
    def compute_metrics(p):
        # --- 데이터 준비 (원본 형태 유지) ---
        logits_list = [paddle.to_tensor(t).astype("float32").cuda() for t in p.predictions]
        # 'labels_original'이라는 이름으로 원본 레이블 텐서 리스트를 만듭니다.
        labels_original_list = [paddle.to_tensor(t).cuda() for t in p.label_ids]

        total_loss = 0
        total_valid_tokens = 0

        # --- "샘플별 계산" 시작 ---
        # zip을 사용하여 로짓과 레이블 짝을 순회
        for i in range(len(logits_list)):
            logits = logits_list[i]        # i번째 샘플의 로짓 (430, 103424)
            labels = labels_original_list[i] # i번째 샘플의 원본 레이블 (430)

            # 손실 계산을 위해 reshape
            reshaped_logits = paddle.reshape(logits, [-1, logits.shape[-1]])
            reshaped_labels = paddle.reshape(labels, [-1])
            
            # [핵심] i번째 샘플에 대한 총 손실 계산
            sample_loss = F.cross_entropy(
                input=reshaped_logits,
                label=reshaped_labels,
                ignore_index=-100,
                reduction="sum"
            )
            
            # i번째 샘플의 유효 토큰 수 계산
            sample_valid_tokens = (reshaped_labels != -100).astype("int64").sum()

            # 전체 합계에 더하기
            total_loss += sample_loss
            total_valid_tokens += sample_valid_tokens
        
        # --- 최종 평균 계산 ---
        if total_valid_tokens.item() == 0:
            return {"nll_loss": 0.0, "ppl": 1.0, "num_token": 0.0}

        nll_loss = total_loss / total_valid_tokens
        ppl = paddle.exp(nll_loss)

        return {
            "nll_loss": nll_loss.item(),
            "ppl": ppl.item(),
            "num_token": total_valid_tokens.item(),
        }
    ...
    if finetuning_args.do_train:
        ...
        finetuning_args.is_train_mm = bool(
            getattr(data_args, "train_dataset_path", None)
            or getattr(data_args, "train_lmdb_paths", None)
        )
        ...
        if finetuning_args.need_data:
            if finetuning_args.multimodal and finetuning_args.is_train_mm:
                # normalize potential LMDB paths passed via CLI as "./mnt/..."
                if getattr(data_args, "train_lmdb_paths", None) and isinstance(
                    data_args.train_lmdb_paths, str
                ):
                    if data_args.train_lmdb_paths.startswith("./mnt/"):
                        data_args.train_lmdb_paths = (
                            "/" + data_args.train_lmdb_paths.lstrip("./")
                        )
                # If user passed LMDB via train_dataset_path (CLI), treat it as LMDB
                train_ds_path_cli = getattr(data_args, "train_dataset_path", None)
                if isinstance(train_ds_path_cli, str):
                    if train_ds_path_cli.startswith("./mnt/"):
                        train_ds_path_cli = "/" + train_ds_path_cli.lstrip("./")
                use_lmdb_via_cli = (
                    isinstance(train_ds_path_cli, str)
                    and (
                        train_ds_path_cli.endswith(".lmdb")
                        or os.path.isdir(train_ds_path_cli)
                    )
                )
                if getattr(data_args, "train_lmdb_paths", None) or use_lmdb_via_cli:
                    # LMDB-based multimodal dataset
                    train_dataset = SFTMultimodalDatasetLmdb(
                        finetuning_args,
                        train_lmdb_paths=(
                            data_args.train_lmdb_paths
                            if getattr(data_args, "train_lmdb_paths", None)
                            else train_ds_path_cli
                        ),
                        train_lmdb_prob=(
                            data_args.train_lmdb_prob
                            if getattr(data_args, "train_lmdb_prob", None)
                            else getattr(data_args, "train_dataset_prob", "1.0") or "1.0"
                        ),
                        tokenizer=tokenizer,
                    ...
                    # JSON/JSONL-based multimodal dataset (default)
    ...
    # Build eval_dataset when needed
    if finetuning_args.do_eval and finetuning_args.need_data:
        if finetuning_args.multimodal and getattr(data_args, "eval_dataset_path", None):
            eval_path = str(data_args.eval_dataset_path)
            # normalize "./mnt/..." to "/mnt/..."
            if eval_path.startswith("./mnt/"):
                eval_path = "/" + eval_path.lstrip("./")
            # LMDB eval if path points to a lmdb dir/file, otherwise JSON/JSONL
            if eval_path.endswith(".lmdb") or os.path.isdir(eval_path):
                eval_mm = SFTMultimodalDatasetLmdb(
                    finetuning_args,
                    train_lmdb_paths=eval_path,
                    train_lmdb_prob=getattr(data_args, "eval_dataset_prob", "1.0") or "1.0",
                    tokenizer=tokenizer,
                    image_preprocess=image_preprocess,
                    seed=finetuning_args.seed,
                    image_token_len=64,
                    ...
                    batch_size=finetuning_args.per_device_eval_batch_size,
                    dp_rank=finetuning_args.reeao_dataset_rank,
                    dp_size=finetuning_args.reeao_dataset_world_size,
                    data_processor=data_processor,
                    need_prefix=False,
                    max_samples_per_source=5,
                )
                eval_mm._load(shuffle_json=False)
            else:
                eval_mm = SFTMultimodalDatasetJson(
                    finetuning_args,
                    train_dataset_path=eval_path,
                    train_dataset_prob=getattr(data_args, "eval_dataset_prob", "1.0"),
                    tokenizer=tokenizer,
                    image_preprocess=image_preprocess,
                    seed=finetuning_args.seed,
                    image_token_len=64,
                ...
                eval_mm._load(shuffle_json=False)
            eval_dataset = IterDataset(
                MixExampleSetJson(
                    finetuning_args,
                    lm_weights=0.0,
                    mm_weights=1.0,
                    lm_example_set=None,
                    mm_example_set=eval_mm,
                )
            )
        # Cap evaluation steps to avoid long/expensive runs on IterableDataset
        if getattr(finetuning_args, "eval_iters", -1) is None or finetuning_args.eval_iters < 0:
            finetuning_args.eval_iters = 5
    ...
    if finetuning_args.do_eval:
        # For evaluation, disable sparse head to ensure full-vocab logits shape for stable CE
